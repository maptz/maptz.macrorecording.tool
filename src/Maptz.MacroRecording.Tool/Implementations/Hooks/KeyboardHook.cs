...
namespace Maptz.MacroRecording.Tool
{
    ...
    public class KeyboardHook : IKeyboardHook
    {
        ...
        {
            var isCanclled = false;
            if (nCode >= 0)
            {
                int iwParam = wParam.ToInt32();
                if ((iwParam == WM_KEYDOWN || iwParam == WM_SYSKEYDOWN))
                    if (KeyDown != null)
                        isCanclled |=  KeyDown((VKeys)Marshal.ReadInt32(lParam));
                if ((iwParam == WM_KEYUP || iwParam == WM_SYSKEYUP))
                    if (KeyUp != null)
                        isCanclled |= KeyUp((VKeys)Marshal.ReadInt32(lParam));
            }
            Debug.WriteLine("Key hook. Cancelled: " + isCanclled);
            if (isCanclled) return (System.IntPtr)1;
